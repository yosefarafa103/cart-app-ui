...
export default function CommentMangement() {
    ...
    const { mutate, isPending } = useMutation({
        mutationFn: async (id) => {
            return await axios.patch(`${localHost}/comments/${id}`, { isDeleted: true })
        }
    })
    const queryClient = useQueryClient()
    const nonDeletedComments = comments?.documents.filter(el => !el.isDeleted);
    console.log(nonDeletedComments);
    
    return (
        <>
            {nonDeletedComments?.length ?
                <div className="my-3">
                    <h3 className="text-[22px] font-bold mb-5">Mange All Comments ( {nonDeletedComments?.length} ) </h3>
                        ...
                        {nonDeletedComments?.map((comment) => (
                            <form onSubmit={(e) => {
                                ...
                                <h2 className="flex gap-3 justify-center mt-4 items-center flex-col">
                                    <img src={userImage} className="size-[100px] object-cover" alt="" />
                                    <div className="font-semibold "><span className="px-3 rounded-md  bg-[#eeeeeeb0]">@{comment?.publishedBy.name}</span> </div>
                                </h2>
                                <p className="mt-4 mb-2 leading-[1] p-2 text-center rounded-md bg-[#eee]">{comment?.text}</p>
                                <button
