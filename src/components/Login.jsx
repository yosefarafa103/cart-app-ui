...
function Login() {
    ...
    if (tryToLoginTimes === 3) {
        Cookies.set("blocked", "true", { expires: new Date(new Date().getTime() + 15 * 60 * 1000) })
    }
    ...
    return (
        <>
            <form onSubmit={handleSubmit((data) => {
                ...
                {Cookies.get("blocked") && <div className="bg-red-400 p-3 mb-3 uppercase rounded-lg text-center text-white font-bold text-sm">you can logged in soon!</div>}
                <div className="flex flex-col gap-1">
                    ...
                    })} type="email" className="px-[10px] py-2 border-solid border-[#ddd] border-[1px] outline-0  rounded-md" placeholder="enter your email" />
                </div>
                ...
                <button disabled={isPending || !!Cookies.get("blocked")} type="submit" className={`block ${(Cookies.get("blocked") || isPending) ? "opacity-[0.5]" : "hover:bg-[#444]"}  transition-all duration-300  rounded-md text-[18px] font-semibold  w-full bg-black text-white p-[10px] mt-[10px]`}>Login</button>
